...
Once this generation concludes the population will go through the series of phases outlined in figure~\ref{fig:customGAcyclee}; altering the population to keep the best performers against our opponent to continue on to subsequent generations.
...
It leverages the use of the function \mintinline{python}{runGeneticAlgo} show in appendix Snippet~\ref{code:runGeneticAlgo}.
...
Each opponent was submitted to the compute engine using the standard analysis factory class \mintinline{python}{AnalysisRun}, shown in Figure~\ref{code:AnalysisRun.py}, using native multi-threading on a Linux OS to improve individual opponent analysis run times and the overall scalability of the project.
